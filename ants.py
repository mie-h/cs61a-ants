...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        if self.min_range != 0:
            count = 0
            while count < self.min_range and rover != beehive:
                rover = rover.entrance
                count += 1
            if rover == beehive:
                return None
        count = 0
        while rover != beehive and count <= self.max_range:
            bee = random_or_none(rover.bees)
            if bee:
                return bee
            else:
                rover = rover.entrance
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting:
            ...
            if chosen_bee:
                Bee.reduce_armor(chosen_bee, chosen_bee.armor)
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if other.is_container is False and not self.contained_ant:
            return True
        else:
            return False
        # END Problem 9
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.impostor = True
        self.list_of_ants = []
        if not QueenAnt.queen_on_field:
            QueenAnt.queen_on_field = True
            self.impostor = False
        # END Problem 13
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        #self.place.remove_insect(self)
        if self.queen_on_field and self.impostor:
            self.reduce_armor(self.armor)
            return
        ThrowerAnt.action(self, colony)
        rover = self.place.exit
        while rover != None:
            temp_ant = rover.ant
            if temp_ant:
                if temp_ant.is_container:
                    if temp_ant not in self.list_of_ants:
                        temp_ant.damage *= 2
                        self.list_of_ants += [temp_ant]
                    if temp_ant.contained_ant and temp_ant.contained_ant not in self.list_of_ants:
                        temp_ant.contained_ant.damage *= 2
                        self.list_of_ants += [temp_ant.contained_ant]
                elif temp_ant not in self.list_of_ants:
                    temp_ant.damage *= 2
                    self.list_of_ants += [temp_ant]
            rover = rover.exit
        # END Problem 13
